...
分類綁定已是三個小分類（處理中／加簽／已結案），與規格 v1.0、現行 `IssueImportTests` 一致；取代上文「兩個大分類／兩個小分類」舊敘述。

| 項目 | 結果 |
|------|------|
| `dotnet test backend.Tests` | 通過 **107**，失敗 0 |
| `Decisions_delete_also_removes_work_hours` | ✅ 結果頁刪除未找到時一併清 `work_hour.issue_id` |
| 本機 API 種子 | ✅ 大分類「議題」；小分類「處理中」「加簽」「已結案」 |
| 前端清單 | 以程式靜態對照＋單元測試勾選（見該檔說明）；未把 Template 全檔打入本機庫 |
