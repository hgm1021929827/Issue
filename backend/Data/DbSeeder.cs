...
{
    private static readonly (string Name, int Sort)[] DefaultMajors =
    [
        ("預約", 1),
        ("議題", 2)
    ];

    private static readonly (string Name, string Color)[] DefaultSubs =
    [
        ("已發信", "#7EB8D8"),
        ("預約完成", "#5BA3C9"),
        ("連線完成", "#6BB3A8"),
    ...
    public static void EnsureSchema(AppDbContext db)
    {
        if (db.Database.IsRelational())
        {
            ...
            AddFkIfMissing(db, "project", "fk_project_owner",
            ...
            AddFkIfMissing(db, "issue", "fk_issue_company",
            ...
            db.Database.ExecuteSqlRaw("""
                ...
                """);
            AddColumnIfMissing(db, "track_todo", "project_work_item_id",
                "ALTER TABLE `track_todo` ADD COLUMN `project_work_item_id` BIGINT NULL");
            AddIndexIfMissing(db, "track_todo", "ix_track_todo_work_item",
                "ALTER TABLE `track_todo` ADD KEY `ix_track_todo_work_item` (`project_work_item_id`, `sort_order`)");
            AddFkIfMissing(db, "track_todo", "fk_track_todo_work_item",
            ...
            DropCheckIfExists(db, "track_todo", "ck_track_todo_work");
            AddCheckIfMissing(db, "track_todo", "ck_track_todo_work", """
    ...
    private static void AddColumnIfMissing(AppDbContext db, string table, string column, string alterSql)
    {
        db.Database.ExecuteSqlRaw($"""
            SET @exists := (
              SELECT COUNT(*) FROM information_schema.COLUMNS
    ...
    private static void AddFkIfMissing(AppDbContext db, string table, string constraint, string alterSql)
    {
        db.Database.ExecuteSqlRaw($"""
            SET @exists := (
    ...
    private static void AddIndexIfMissing(AppDbContext db, string table, string index, string alterSql)
    {
        db.Database.ExecuteSqlRaw($"""
            SET @exists := (
              SELECT COUNT(*) FROM information_schema.STATISTICS
    ...
    private static void DropCheckIfExists(AppDbContext db, string table, string constraint)
    {
        db.Database.ExecuteSqlRaw($"""
            SET @exists := (
              SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS
    ...
    private static void AddCheckIfMissing(AppDbContext db, string table, string constraint, string alterSql)
    {
        db.Database.ExecuteSqlRaw($"""
            SET @exists := (
              SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS
    ...
    {
        EnsureMajors(db);
        EnsureDefaultSubs(db);
        EnsureIssueMajorAndSubs(db);
    }

    public static void EnsureMajors(AppDbContext db)
    {
        if (db.MajorCategories.Any())
        {
            return;
        }
        foreach (var (name, sort) in DefaultMajors)
        {
    ...
    public static void EnsureDefaultSubs(AppDbContext db)
    {
        var keep = KeepNames();
        var majors = db.MajorCategories.Where(x => keep.Contains(x.CategoryName))
        ...
        foreach (var major in majors)
        {
            ...
            foreach (var (name, color) in DefaultSubs)
            {
                var item = existing.FirstOrDefault(x => x.SubCategoryName == name);
                if (item is null)
                {
                    sort += 1;
                ...
                else if (!ColorPresets.IsAllowed(item.ColorHex) || item.ColorHex.Equals("#8FA8C8", StringComparison.OrdinalIgnoreCase))
                {
                    item.ColorHex = color;
                }
            }
    ...
    private static HashSet<string> KeepNames() => ["預約"];
}
