...
這是一個**個人工作追蹤 Web 系統**。第一、三～十三階段核心功能已有應用程式碼；第十三階段「正式議題 Excel 匯入」已開發完成並通過 QA，前端手動清單待使用者勾選。舊 Excel 設計原稿在第四階段歷史資料，不得直接實作。新畫面對齊第二階段視覺規格。
...
第一個功能從「第一階段」開始編號，資料夾加兩位數前綴（`01-`、`02-`…）；新一輪改善建立下一階段，不覆寫已結束階段的正式文件。第十二階段目錄為 `12-第十二階段－專案工作項次與工時/`。第十三階段目錄為 `13-第十三階段－正式議題 Excel 匯入/`。
